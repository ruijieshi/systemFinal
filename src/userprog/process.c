...
/* Pushes the SIZE bytes in BUF onto the stack in cur_page, whose
...
static void *
push (uint8_t *cur_page, size_t *ofs, const void *buf, size_t size)
{
    ...
    *ofs -= padsize;
    memcpy (cur_page + *ofs + (padsize - size), buf, size);
    return cur_page + *ofs + (padsize - size);
}

/* Sets up command line arguments in cur_page, which will be mapped
...
static bool
init_cmd_line (uint8_t *cur_page, uint8_t *upage, const char *cmd_line,
               void **esp)
{
    size_t ofs = PGSIZE;
    ...
    cmd_line_copy = push (cur_page, &ofs, cmd_line, strlen (cmd_line) + 1);
    if (cmd_line_copy == NULL)
        return false;

    if (push (cur_page, &ofs, &null, sizeof null) == NULL)
        return false;
    ...
    {
        void *uarg = upage + (karg - (char *) cur_page);
        if (push (cur_page, &ofs, &uarg, sizeof uarg) == NULL)
            return false;
        argc++;
    }
    ...
    argv = (char **) (upage + ofs);
    reverse (argc, (char **) (cur_page + ofs));

    /* Push argv, argc, "return address". */
    if (push (cur_page, &ofs, &argv, sizeof argv) == NULL
        || push (cur_page, &ofs, &argc, sizeof argc) == NULL
        || push (cur_page, &ofs, &null, sizeof null) == NULL)
        return false;
